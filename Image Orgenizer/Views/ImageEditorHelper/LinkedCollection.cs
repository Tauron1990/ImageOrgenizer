...
namespace ImageOrganizer.Views.ImageEditorHelper
{
    public sealed class LinkedCollection<TData> : IList<TData>, INotifyCollectionChanged, INotifyPropertyChanged, IList
    {
        ...
        public int Add(object value) => ((IList) _linked).Add(value);

        public bool Contains(object value) => ((IList) _linked).Contains(value);

        public void Clear() => _linked.Clear();
        public int IndexOf(object value) => ((IList) _linked).IndexOf(value);

        public void Insert(int index, object value) => ((IList) _linked).Insert(index, value);

        public void Remove(object value) => ((IList) _linked).Remove(value);
        ...
        public void CopyTo(Array array, int index) => ((ICollection) _linked).CopyTo(array, index);
